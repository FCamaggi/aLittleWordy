...
export const Tile: React.FC<TileProps> = ({ tile, onClick, selected, disabled, small, faceDown, highlight }) => {
  ...
  if (tile.disabled) {
    return (
      ...
      >
        ...
        <div className="absolute inset-0 bg-slate-400 opacity-20" style={{ 
  ...
  return (
    ...
    >
      {tile.letter}
      {isRevealed && <span className="absolute top-0 right-0 w-2 h-2 bg-green-500 rounded-full"></span>}
    </div>
  );
};
